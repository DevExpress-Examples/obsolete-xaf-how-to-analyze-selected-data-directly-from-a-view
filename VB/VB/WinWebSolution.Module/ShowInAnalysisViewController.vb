...
Namespace WinWebSolution.Module
	...
	Public Class ShowInAnalysisViewController
		...
		Protected Overrides Overloads Sub UpdateActionActivity(ByVal action As ActionBase)
			...
			Dim visibleInReportsModel As IModelClassReportsVisibility = TryCast(CType(View.Model, IModelObjectView).ModelClass, IModelClassReportsVisibility)
...
Private Sub analysisEditor_DataSourceCreated(ByVal sender As Object, ByVal e As DataSourceCreatedEventArgs)
	Dim dataSource As Object = e.DataSource
	If TypeOf dataSource Is ProxyCollection Then
		dataSource = (CType(dataSource, ProxyCollection)).OriginalCollection
	End If
	Dim collection As XPCollection = TryCast(dataSource, XPCollection)
	If collection IsNot Nothing Then
		collection.Criteria = collection.Criteria And selectionCriteria
	End If
End Sub
